...
## What's here
- `index.html` — camera capture, hand-guide overlay (via MediaPipe, for framing only —
  it does not read lines), onboarding, loading state, and reading display.
- `api/reading.js` — Vercel serverless function that sends the captured photo to
  Claude's vision API and returns a structured reading.
