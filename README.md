...
- `index.html` — camera capture, hand-guide overlay, onboarding, a quick local hand-shape
  preview, the AI reading, a second-hand comparison flow, and a separate face-reading path.
- `api/reading.js` — Vercel serverless function that sends one captured palm photo to
  Claude's vision API and returns a structured reading (8 lines: life, heart, head, fate,
  Sun, Mercury, Girdle of Venus, marriage).
- `api/compare.js` — sends both hands' photos in one call and returns a "born with vs.
  self-made" comparison, line by line.
- `api/face-reading.js` — sends a captured face photo and returns a traditional face-reading
  (forehead, eyebrows, eyes, nose, mouth, chin), with explicit guardrails against any
  race, health, or attractiveness commentary.
- `package.json` — minimal, no dependencies required.

## The flow
**Palm path:**
1. On the intro screen, choose "Palm," then pick your writing hand.
...
4. Tap "Start your analysis" → real AI reading of that hand's actual lines, with each
   line traced as a glowing overlay on your photo.
...
**Face path:**
1. On the intro screen, choose "Face" instead — skips the hand question.
2. Front camera opens with an oval guide; tap the shutter manually (no auto-capture yet,
   since this doesn't use hand-landmark tracking).
3. Goes straight to the AI reading — no quick local preview step for faces.
4. Each feature (forehead, eyebrows, eyes, nose, mouth, chin) gets a small glowing point
   marker on the photo instead of a traced line, revealed alongside its section.

Both paths support English/Hindi via the toggle on the intro screen.
